...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        self.q = BinaryLinear(dim, dim, bias=qkv_bias) #nn.Linear(dim, dim, bias=qkv_bias)
        self.kv = BinaryLinear(dim, dim * 2, bias=qkv_bias) #nn.Linear(dim, dim * 2, bias=qkv_bias)
        ...
        if sr_ratio > 1:
            ...
            self.sr = HardBinaryConv(dim, dim, kernel_size=sr_ratio, stride=sr_ratio)
...
class PyramidVisionBinTransformer(nn.Module):
...
class pvt_bin_tiny(PyramidVisionBinTransformer):
    def __init__(self, **kwargs):
        super(pvt_bin_tiny, self).__init__(
...
@BACKBONES.register_module()
class pvt_bin_small(PyramidVisionBinTransformer):
    def __init__(self, **kwargs):
        super(pvt_bin_small, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4],
...
@BACKBONES.register_module()
class pvt_bin_small_f4(PyramidVisionBinTransformer):
    def __init__(self, **kwargs):
        super(pvt_bin_small_f4, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 6, 3],
